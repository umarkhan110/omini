...
const SuspendUserModal = ({ visible, onCancel }) => {
  const [form] = Form.useForm();

  const handleSuspendUser = async () => {
    try {
      const values = await form.validateFields();
      console.log("Suspend user:", values);
      // Perform the logic to add the user
      const response = await fetch(
        `https://dashboard.postgresbc.info/cgi-bin/pgbc_api/suspend_user?manager_user=${values.managerUsername}&manager_password=${values.managerPassword}&user=${values.username}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <Modal
      ...
      footer={[
        ...
        <Button key="add" type="primary" onClick={handleSuspendUser}>
          Suspend
        </Button>,
      ]}
    >
